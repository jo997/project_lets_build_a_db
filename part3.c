// part 3 we added an An In-Memory, Append-Only, Single-Table Database
// right now we have a lot of limitations on our database:
...
// prepare_statement (our “SQL Compiler”) does not understand SQL right now., only understands select and insert right now
...
We’ll start with something simpler. Like a B-tree, it will group rows into pages, 
...
// page size 4 kilobytes because it’s the same size as a page used in the virtual memory systems of most computer architectures
...
// how we figure out where to read/write in memory for a particular row
